...
def handle_form_submission(patient_name, age, gender, pdf_contents):
    ...
    with st.spinner("Analyzing report..."):
        ...
        if result["success"]:
            ...
            # Extract and remove the doctor's advice script from display content
            display_content = content
            advice_script = ""
            if "### Doctor's Advice Script" in content:
                advice_start = content.find("### Doctor's Advice Script")
                advice_end = content.find("###", advice_start + 1) if "###" in content[advice_start + 1:] else len(content)
                advice_script = content[advice_start:advice_end].strip()
                display_content = content[:advice_start] + (content[advice_end:] if advice_end < len(content) else "")
            
            # Display the English analysis without the advice script
                st.markdown(display_content)
                
                # Translate to Urdu and generate speech
                # Function ka naam 'translate_to_urdu' se 'translate_to_hindi' kar do
                hindi_text = st.session_state.tts_service.translate_to_hindi(advice_script)
                # urdu_text = st.session_state.tts_service.translate_to_urdu(advice_script)
                if hindi_text:
                    audio_content = st.session_state.tts_service.generate_speech(hindi_text)
                    if audio_content:
